...
class ConsoleTransferListener(out: PrintStream = System.out) extends AbstractTransferListener {
  ...
  override def transferSucceeded(event: TransferEvent) {
    ...
    if (contentLength >= 0) {
      ...
      out.println(t + ": " + resource.getRepositoryUrl() + resource.getResourceName() + " (" + len
        + throughput + ")");
    }
  }
